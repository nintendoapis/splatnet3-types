import { VsHistoryDetail, VsPlayer, VsPlayer_Gear, VsTeam, VsTeamResult } from './battles.js';
...
type VsHistoryDetail_result = Pick<VsHistoryDetail, '__typename' | 'id' | 'vsRule' | 'vsMode' | 'judgement' | 'vsStage' | 'festMatch' | 'knockout' | 'bankaraMatch' | 'xMatch' | 'duration' | 'playedTime' | 'awards' | 'leagueMatch' | 'nextHistoryDetail' | 'previousHistoryDetail'> & {
    player: VsPlayer_result;
    myTeam: VsTeam_result;
    otherTeams: VsTeam_result[];
};
type VsPlayer_result = Pick<VsPlayer, '__isPlayer' | 'id' | 'name' | 'nameId' | 'byname' | 'nameplate' | 'headGear' | 'clothingGear' | 'shoesGear' | 'paint'>;
type VsTeam_result = Pick<VsTeam, 'color' | 'judgement' | 'tricolorRole' | 'festTeamName' | 'order'> & {
    result: Pick<VsTeamResult, 'paintRatio' | 'score' | 'noroshi'> | null;
    players: VsPlayer_result_VsTeam[];
};

type VsPlayer_result_VsTeam = Pick<VsPlayer, '__typename' | '__isPlayer' | 'id' | 'name' | 'nameId' | 'byname' | 'isMyself' | 'weapon' | 'species' | 'nameplate' | 'paint' | 'result' | 'festDragonCert'> & {
    headGear: VsPlayer_Gear<HeadGear>;
    clothingGear: VsPlayer_Gear<ClothingGear>;
    shoesGear: VsPlayer_Gear<ShoesGear>;
};
...
type VsHistoryDetail_pagerRefetch = Pick<VsHistoryDetail, '__typename' | 'nextHistoryDetail' | 'previousHistoryDetail' | 'id'>;
